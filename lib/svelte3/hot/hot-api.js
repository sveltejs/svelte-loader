...
// Native overrides this with a global because we can't import anything from
// the native adapter, even dynamically, because the native adapter requires
// modules from NativeScript's core that are not resolvable for non-native
// users (and those missing modules would prevent webpack from building).
const ProxyAdapter = global.__SvelteLoader__ProxyAdapter || DomAdapter;

const createProxy = initProxy(ProxyAdapter);
